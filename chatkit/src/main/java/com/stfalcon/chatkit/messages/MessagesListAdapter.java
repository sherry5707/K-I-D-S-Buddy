...
//        if (reverse) Collections.reverse(messages);
    ...
    private void generateDateHeaders(List<MESSAGE> messages) {
        ...
        for (int i = 0; i < messages.size(); i++) {
            ...
            Log.d("MessagersListAdapter:", i + "--" + message.getCreatedAt());
            if (i == 0) {
            ...
            if (messages.size() > i + 1) {
                ...
                if (DateFormatter.isMoreTenMin(message.getCreatedAt(), nextMessage.getCreatedAt())) {
                    this.items.add(new Wrapper<>(nextMessage.getCreatedAt()));
                }
                this.items.add(new Wrapper<>(nextMessage));
            }
    ...
    public void notifyData() {
        ...
        if (scrollToEndListenner != null&&!items.isEmpty())
            scrollToEndListenner.scrollToposition(items.size() - 1);
    }
    ...
    public static class DefaultDateHeaderViewHolder extends ViewHolder<Date>
        ...
        public void onBind(Date date, MessagesListStyle messagesListStyle) {
            if (text != null) {
                ...
                try {
                    IsYesterday = IsYesterday(DateFormatter.format(date, "yyyy-MMMM-dd"));
                ...
                try {
                    IsToday = IsToday(DateFormatter.format(date, "yyyy-MMMM-dd"));
                } catch (ParseException e) {
                    e.printStackTrace();
                }
    ...
    public void setScrollToEndListenner(ScrollToEndListenner scrollToEndListenner) {
        this.scrollToEndListenner = scrollToEndListenner;

    }

    ScrollToEndListenner scrollToEndListenner;

    /**当聊天内容刷新时，通知ChatActivity 去主动滑到最后一行。
     */
    public interface ScrollToEndListenner {
        void scrollToposition(int items);
