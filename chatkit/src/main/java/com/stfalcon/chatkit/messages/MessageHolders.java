 ...
 * knight.xu support local image display, message direction
 */
...
public class MessageHolders {
    ...
    protected void bind(final ViewHolder holder, final Object item, boolean isSelected,
                        ...
                        final MessagesListStyle messagesListStyle) {
        ...
        holder.onBind(item, messagesListStyle);
    }
    ...
    protected int getViewType(Object item, String senderId) {
        ...
        if (item instanceof IMessage) {
            ...
            isOutcoming = message.isOutComming();
     ...
     */
    private static int clickPostion;
    private static ImageButton clickButton;
    ...
    public static class IncomingTextMessageViewHolder<MESSAGE extends IMessage>
        ...
        protected int getClicPosition() {
            return clickPostion;
        }
        protected ImageButton getClicButton() {
            return clickButton;
        }
        protected void setClickPosition(int position){
            clickPostion = position;
        }
        protected void setClickButton(ImageButton clickButton1){
            clickButton = clickButton1;
        }

        @Override
        public void onBind(MESSAGE message, MessagesListStyle messagesListStyle) {
     ...
     */
    private static int clickPostions;
    private static ImageButton clickButtons;
    public static class OutcomingTextMessageViewHolder<MESSAGE extends IMessage>
        ...
        protected TextView text;
        protected int getClicPosition() {
            return clickPostions;
        }
        protected ImageButton getClicButton() {
            return clickButtons;
        }
        protected void setClickPosition(int position){
            clickPostions = position;
        }
        protected void setClickButton(ImageButton clickButton1){
            clickButtons = clickButton1;
        }
    ...
    public static class IncomingImageMessageViewHolder<MESSAGE extends MessageContentType.Image>
        ...
        public void onBind(MESSAGE message, MessagesListStyle messagesListStyle) {
            ...
            if (image != null && imageLoader != null) {
                if (!TextUtils.isEmpty(message.getImageLocalPath())) {
                    imageLoader.loadLocalImage(image, message.getImageLocalPath());
                } else if (!TextUtils.isEmpty(message.getImageUrl())) {
    ...
    public static class DefaultDateHeaderViewHolder extends ViewHolder<Date>
        ...
        @Override
        public void onBind(Date date, MessagesListStyle messagesListStyle) {
            if (text != null) {
...
//                text.setText(formattedDate == null ? DateFormatter.format(date, dateFormat) : formattedDate);
                ...
                if (IsToday) {
                    String today = getFormartDateStr("HH:mm", date);
                    text.setText(today);                  //modify to 24 hour format
            /*        String amOrPm = getFormartDateStr("a", date);
                    if (amOrPm.equals("上午")) {
                        String h = getFormartDateStr("HH", date);
                        int a = Integer.parseInt(h);
                        if (a >= 0 && a <= 6) {
                            if (a == 0) {
                                String DayBreak = getFormartDateStr("00:mm", date);
                                text.setText(DayBreak);//("凌晨" + DayBreak);
                            } else {
                                text.setText(today);//("凌晨" + today);
                            }
                        } else {
                            text.setText(today);//("上午" + today);
                        }
                    } else {
                        String h = getFormartDateStr("HH", date);
                        int a = Integer.parseInt(h);
                        if (a > 12 && a <= 18) {
                            text.setText(today); //("下午" + today);
                        } else {
                            text.setText("晚上" + today);
                        }
                    }*/

                } else if (IsYesterday) {
                    String yesterday = getFormartDateStr("HH:mm", date);
                    text.setText("昨天" + yesterday);
                /*    String amOrPm = getFormartDateStr("a", date);
                    if (amOrPm.equals("上午")) {
                        String h = getFormartDateStr("HH", date);
                        int a = Integer.parseInt(h);
                        if (a >= 0 && a <= 6) {
                            if (a == 0) {
                                String dayBreak = getFormartDateStr("00:mm", date);
                                text.setText("昨天凌晨" + dayBreak);
                            } else {
                                text.setText("昨天凌晨" + yesterday);
                            }
                        } else {
                            text.setText("昨天上午" + yesterday);
                        }
                    } else {
                        String h = getFormartDateStr("HH", date);
                        int a = Integer.parseInt(h);
                        if (a > 12 && a <= 18) {
                            text.setText("昨天下午" + yesterday);
                        } else {
                            text.setText("昨天晚上" + yesterday);
                        }
                    }*/
                } else {
                    String dayBreak = getFormartDateStr("yyyy-MM-dd HH:mm", date);
                    text.setText(dayBreak);
               /*     String amOrPm = getFormartDateStr("a", date);
                    if (amOrPm.equals("上午")) {
                        String h = getFormartDateStr("HH", date);
                        int a = Integer.parseInt(h);
                        if (a >= 0 && a <= 6) {
                            if (a == 0) {
                                String dayBreak = getFormartDateStr("yyyy-MM-dd '凌晨'00:mm", date);
                                text.setText(dayBreak);
                            } else {
                                String dayBreak = getFormartDateStr("yyyy-MM-dd '凌晨'hh:mm", date);
                                text.setText(dayBreak);
                            }
                        } else {
                            String am = getFormartDateStr("yyyy-MM-dd '上午'hh:mm", date);
                            text.setText(am);
                        }
                    } else {
                        String h = getFormartDateStr("HH", date);
                        int a = Integer.parseInt(h);
                        if (a > 12 && a <= 18) {
                            String pm = getFormartDateStr("yyyy-MM-dd '下午'hh:mm", date);
                            text.setText(pm);
                        } else {
                            String night = getFormartDateStr("yyyy-MM-dd '晚上'hh:mm", date);
                            text.setText(night);
                        }
                    } */
                }
        ...
        public void applyStyle(MessagesListStyle style) {
            ...
            dateFormat = dateFormat == null ? DateFormatter.Template.STRING_DAY_MONTH_CHAT_HEADER_TIME.get() : dateFormat;
        }
    }
    ...
    public static SimpleDateFormat getDateFormat() {
        if (null == DateLocal.get()) {
            DateLocal.set(new SimpleDateFormat("MM月dd日", Locale.CHINA));
        }
